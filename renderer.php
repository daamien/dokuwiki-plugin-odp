 ...
 * The Renderer
 */
class renderer_plugin_odp extends Doku_Renderer {
    var $ZIP = null;
    var $meta;
    var $store = '';
    var $footnotes = array();
	...
	$this->toc = '';
	$this->toc_counter = 0;
	$this->toc_source = '<text:table-of-content-source text:outline-level="4">';
	$this->toc_source .= '<text:index-title-template text:style-name="Contents_20_Heading">Table des matières</text:index-title-template>';

	$this->toc_index = '<text:index-title text:style-name="Sect2" text:name="Table des matières1_Head">';
	$this->toc_index .= '<text:p text:style-name="Contents_20_Heading">Table des matières</text:p>';
 	$this->toc_index .= '</text:index-title>';

        //$headers = array('Content-Type'=>'text/plain'); p_set_metadata($ID,array('format' => array('odp' => $headers) )); return ; // DEBUG
        // send the content type header, new method after 2007-06-26 (handles caching)
        $output_filename = str_replace(':','-',$ID).".odp";
        if (version_compare($dw_version, "20070626")) {
            ...
            p_set_metadata($ID,array('format' => array('odp' => $headers) ));
        } else { // older method
    ...
    function _odpMeta(){
        ...
        $value .=   'xmlns:presentation="urn:oasis:names:tc:opendocument:xmlns:presentation:1.0" ';
        $value .=   'xmlns:ooo="http://openoffice.org/2004/office" ';
        $value .=   'xmlns:smil="urn:oasis:names:tc:opendocument:xmlns:smil-compatible:1.0" ';
        $value .=   'xmlns:anim="urn:oasis:names:tc:opendocument:xmlns:animation:1.0" ';
        ...
        $value .=   'grddl:transformation="http://docs.oasis-open.org/office/1.2/xslt/odf2rdf.xsl" >';
        $value .=       '<office:meta>';
    # FIXME
     ...
     */
    function _odpManifest(){
        $value  =   '<' . '?xml version="1.0" encoding="UTF-8"?' . ">\n";
        $value .=   '<manifest:manifest xmlns:manifest="urn:oasis:names:tc:opendocument:xmlns:manifest:1.0">';
        $value .=   '<manifest:file-entry manifest:media-type="application/vnd.oasis.opendocument.presentation" manifest:version="1.2" manifest:full-path="/"/>';
     ...
     */
    function _odpSettings(){
        $value  =   '<' . '?xml version="1.0" encoding="UTF-8"?' . ">\n";
        $value .=   '<office:document-settings ';
	$value .=   'xmlns:office="urn:oasis:names:tc:opendocument:xmlns:office:1.0" ';
	$value .=   'xmlns:xlink="http://www.w3.org/1999/xlink" ';
	$value .=   'xmlns:config="urn:oasis:names:tc:opendocument:xmlns:config:1.0" ';
	$value .=   'xmlns:presentation="urn:oasis:names:tc:opendocument:xmlns:presentation:1.0" ';
	$value .=   'xmlns:ooo="http://openoffice.org/2004/office" ';
	$value .=   'xmlns:smil="urn:oasis:names:tc:opendocument:xmlns:smil-compatible:1.0" '; 
	$value .=   'xmlns:anim="urn:oasis:names:tc:opendocument:xmlns:animation:1.0" ';
	...
	$value .=   '<config:config-item-set config:name="dummy-settings">';
	$value .=   '<config:config-item config:name="MakeValidatorHappy" config:type="boolean">';
	$value .=   'true';
	$value .=   '</config:config-item>';
	$value .=   '</config:config-item-set>';
	$value .=   '</office:settings>';
	$value .=   '</office:document-settings>';
    ...
    function document_end(){
        global $conf;
        //$this->doc .= $this->_odpAutoStyles(); return; // DEBUG
	...
	$value .=   		'</draw:page>';
        ...
        $value = io_readFile(DOKU_PLUGIN.'odp/styles.xml');
    ...
    function document_end_template(){
        ...
        $this->temp_dir = $temp_dir."/odp/".str_replace(':','-',$ID);
        ...
        // Prepare content
        $autostyles = $this->_odpAutoStyles();
        $missingstyles = $this->_odpStyles();
        $missingfonts = $this->_odpFonts();
        $userfields = $this->_odpUserFields();

        // Insert content
        $old_content = io_readFile($this->temp_dir.'/content.xml');
        if (strpos($old_content, 'DOKUWIKI-ODP-INSERT') !== FALSE) { // Replace the mark
	 ...
	 $this->render_TOC();
        ...
        if (strpos($old_content, 'DOKUWIKI-ODP-CUT-START') !== FALSE 
                && strpos($old_content, 'DOKUWIKI-ODP-CUT-STOP') !== FALSE) {
            $this->_odpReplaceInFile('/DOKUWIKI-ODP-CUT-START.*DOKUWIKI-ODP-CUT-STOP/', 
                '', $this->temp_dir.'/content.xml', true);
        }

        // Insert userfields
        if (strpos($old_content, "text:user-field-decls") === FALSE) { // no existing userfields
            $this->_odpReplaceInFile('/<office:text([^>]*)>/U', '<office:text\1>'.$userfields, $this->temp_dir.'/content.xml', TRUE);
        } else {
            $this->_odpReplaceInFile('</text:user-field-decls>', substr($userfields,23), $this->temp_dir.'/content.xml');
        }

        // Insert styles & fonts
        $this->_odpReplaceInFile('</office:automatic-styles>', substr($autostyles, 25), $this->temp_dir.'/content.xml');
        $this->_odpReplaceInFile('</office:automatic-styles>', substr($autostyles, 25), $this->temp_dir.'/styles.xml');
        $this->_odpReplaceInFile('</office:styles>', $missingstyles.'</office:styles>', $this->temp_dir.'/styles.xml');
        $this->_odpReplaceInFile('</office:font-face-decls>', $missingfonts.'</office:font-face-decls>', $this->temp_dir.'/styles.xml');
    ...
    function _odpReplaceInFile($from, $to, $file, $regexp=FALSE) {
...
*/
    ...
    function _highlight($type, $text, $language=null) {
        ...
        $highlighted_code = preg_replace_callback('/<span style="([^"]+)">/', array('renderer_plugin_odp','_convert_css_styles'), $highlighted_code);
